...
import { portalApi, ApiError } from "@/lib/portal-api-client";
...
export default function PortalForgotPasswordPage() {
  ...
  const onSubmit = async (values: ForgotPasswordForm) => {
    try {
      await portalApi.post("/portal/auth/forgot-password", values, { skipAuth: true });
      setSent(true);
    } catch (e) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            If an activated account exists for that email, we&apos;ve sent a link to reset your password. It expires
            in 30 minutes.
          </CardDescription>
        </CardHeader>
        <CardContent>
